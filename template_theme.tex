...
\newcommand\startmyframe{
  \begin{centering}
    \begin{tikzpicture}
      ...
      % Current coordinate
      \coordinate (current) at (\xlmargin ,{\paperheight-\headerheight});
  ...
  \begin{frame}[plain]
    \begin{centering}
      \begin{tikzpicture}
        ...
        % Subtitle
        \node[anchor = west, color=\maincolor] (subtitle) at (\covermargin,{.35*\paperheight}) {\textbf{#2}};
        % Title
        \node[anchor = south west, color=black] (title) at ($(subtitle.north west) + (0,.1cm)$) {\Large \textbf{#1}};
        ...
        % Date
        \node[anchor = north west, color=\secondcolor!60] (date) at ($(subtitle.south west) - (0,.1cm)$) {\textbf{#3}};
        % Authors
